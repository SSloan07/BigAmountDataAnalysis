...
const Persona* Buscar::encontrarPersonaMasRica(const std::vector<Persona*>& grupo) {
    ...
    double maxPatrimonio = personaMasRica->patrimonio;

    for (const auto& persona : grupo) {
        double patrimonioActual = persona->patrimonio;
...
void Buscar::mostrarTop3CiudadesPatrimonio(const std::vector<Persona>& personas) {
    ...
    for (const auto& ciudad : ciudadesColombia) {
        ...
        for (const auto& persona : personas) {
            if (persona.ciudadNacimiento == ciudad) {
                total += persona.patrimonio;
                contador++;
            }
        }
...
void Buscar::contarPersonaPatrimonioMayor1000Millones(const std::vector<Persona>& personas) {
    ...
    for (const auto& persona : personas) {
        if (persona.patrimonio >= 1000000000) {  // 1000 millones
    ...
    auto compararPatrimonio = [](const Persona* a, const Persona* b) {
        return a->patrimonio < b->patrimonio;  // Esto de acá es una función lambda que recibe el inicio y fin del vector filtrados. Y en esta lina de codigo compara si el patrimonio de la persona anterior es menor a la posterior, en tal caso devuelve true, sino devuelve false. 
    };
...
void Buscar::personaConMayorPatrimonio(const std::vector<Persona>& personas) {
    ...
    double maxPatrimonioGlobal = personaMasRicaGlobal->patrimonio;
